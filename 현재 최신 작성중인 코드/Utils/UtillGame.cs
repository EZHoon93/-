...
public static class UtillGame
{
    ...
    public static Vector3 GetStraightHitPoint(Vector3 startPoint, Vector3 inputVector3, float distance,LayerMask layerMask )
    {
        RaycastHit hit;
        Vector3 hitPosition;
        startPoint.y = 0.5f;
        ...
        if (Physics.Raycast(startPoint, direction, out hit, distance, layerMask))
        {
            ...
            hitPosition.y = startPoint.y;
        }
        else
        {
            hitPosition = startPoint + direction * distance;
        }
        //hitPosition.y = 0.5f;
        hitPosition.y = startPoint.y;    //높이를 갖게.
        return hitPosition;
    }
    ...
    public static Vector3 GetCurveHitPoint(Plane plane,Transform startTransform, Vector2 inputVector2, float distance)
    {
        ...
        if (plane.Raycast(ray, out hitDist))
        {
            targetPoint = ray.GetPoint(hitDist);
            RaycastHit hitInfo;
            //if (Physics.Linecast(startPoint, targetPoint, out hitInfo, 1 << (int)Define.Layer.TransparentFX))
            //{
            //    targetPoint = hitInfo.point;
            //}
        }
    ...
    public static int DamageInRange(Vector3 center, float radius, int damage, int damagerViewID,
        LayerMask attackLayer,Define.Team team)
    {
            ...
            for (int i = 0; i < hitCount; i++)
            {
                ...
                if (damageable != null )
                {
                    if (damageable.Team == team)
                        continue;
                    damageable.ReceiveAnAttack(damagerViewID, damage, colliders[i].transform.position);
                }
            }
        }
    ...
    public static T GetRandomItem<T>(ref List<T> pointList)
    {
        if (pointList.Count <= 0)
        {
            return default(T);
        }
        var ranIndex = Random.Range(0, pointList.Count);
        var go = pointList[ranIndex];
        pointList.RemoveAt(ranIndex);
        return go;
    }
    public static T[] GetRandomItems<T>(T[] arrayItem, int count)
    {
        if(arrayItem.Length < count)
        {
            Debug.LogError("Count 더적음");
            return null;
        }

        var list = arrayItem.ToList();
        var result = new List<T>();
        for(int i =0; i <count; i++)
        {
            var select = GetRandomItem(ref list);
            result.Add(select);
        }
    ...
    public static T GetRandomItem<T>(T[] arrayItem)
    {
        if (arrayItem.Length <= 0)
        {
            return default(T);
        }
        var ranIndex = Random.Range(0, arrayItem.Length);
        var go = arrayItem[ranIndex];
        return go;
    }
